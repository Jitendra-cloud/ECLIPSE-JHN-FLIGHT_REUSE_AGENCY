...
CLASS ltc_agency_handler DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    ...
    DATA:
      mapped        TYPE RESPONSE FOR MAPPED   EARLY ZJS_R_AgencyTP,
      failed        TYPE RESPONSE FOR FAILED   EARLY ZJS_R_AgencyTP,
      reported      TYPE RESPONSE FOR REPORTED EARLY ZJS_R_AgencyTP,
      failed_late   TYPE RESPONSE FOR FAILED   LATE  ZJS_R_AgencyTP,
      reported_late TYPE RESPONSE FOR REPORTED LATE  ZJS_R_AgencyTP.
...
CLASS ltc_agency_handler IMPLEMENTATION.
  ...
  METHOD class_setup.
    ...
    sql_test_environment = cl_osql_test_environment=>create( i_dependency_list = VALUE #( ( 'ZJS_R_AgencyTP' ) ( 'I_COUNTRY' ) ) ).
  ENDMETHOD.
  ...
  METHOD validate_name_success.
    DATA:
      agency   TYPE ZJS_R_AgencyTP,
      agencies TYPE STANDARD TABLE OF ZJS_R_AgencyTP.

    agency = VALUE ZJS_R_AgencyTP(
        AgencyId = '123'
        Name     = 'FlightBooker'
      ).
    ...
    cl_abap_unit_assert=>assert_equals(
        ...
        exp = 1
        act = lines( reported_late-Agency )
      ).
    DATA(reported_agency) = reported_late-Agency[ 1 ].
  ...
  METHOD validate_name_failed.
    DATA:
      agency                    TYPE ZJS_R_AgencyTP,
      agencies                  TYPE STANDARD TABLE OF ZJS_R_AgencyTP,
      reported_with_message     TYPE STRUCTURE FOR REPORTED LATE ZJS_R_AgencyTP,
      reported_clear_state_area TYPE STRUCTURE FOR REPORTED LATE ZJS_R_AgencyTP.
    ...
    cl_abap_unit_assert=>assert_equals( exp = 1  act = lines( failed_late-Agency ) ).
    ...
    LOOP AT reported_late-Agency INTO DATA(reported_line).
    ...
    cl_abap_unit_assert=>assert_equals(
        exp = CORRESPONDING symsg( zjs_cx_agency=>name_required )
  ...
  METHOD validate_email_failed.
    CONSTANTS: cv_found TYPE ZJS_R_AgencyTP-Name VALUE 'found' ##NO_TEXT.
    DATA:
      agency                    TYPE ZJS_R_AgencyTP,
      agencies                  TYPE STANDARD TABLE OF ZJS_R_AgencyTP,
      reported_with_message     TYPE STRUCTURE FOR REPORTED LATE ZJS_R_AgencyTP,
    ...
    LOOP AT agencies ASSIGNING FIELD-SYMBOL(<agency>).
      ...
      LOOP AT reported_late-Agency INTO DATA(reported_line) USING KEY entity WHERE AgencyId = <agency>-AgencyId.
        IF reported_line-%msg IS BOUND.
          reported_with_message     = reported_line.
        ELSE.
          reported_clear_state_area = reported_line.
      ...
      cl_abap_unit_assert=>assert_equals(
          exp = CORRESPONDING symsg( zjs_cx_agency=>email_invalid_format )
          act = CORRESPONDING symsg( reported_with_message-%msg->if_t100_message~t100key )
        ).
    ENDLOOP.
...
*    cds_test_environment->insert_test_data( agencies ).
  ...
  METHOD validate_countrycode_failed.
    ...
    LOOP AT agencies ASSIGNING FIELD-SYMBOL(<agency>).
      ...
      cl_abap_unit_assert=>assert_equals(
          exp = CORRESPONDING symsg( zjs_cx_agency=>country_code_invalid )
          act = CORRESPONDING symsg( reported_with_message-%msg->if_t100_message~t100key )
        ).
    ENDLOOP.
  ...
  METHOD get_global_authorizations.
    DATA:
      requested_authorizations TYPE STRUCTURE FOR GLOBAL AUTHORIZATION REQUEST ZJS_R_AgencyTP\\Agency,
      result                   TYPE STRUCTURE FOR GLOBAL AUTHORIZATION RESULT ZJS_R_AgencyTP\\Agency,
      reported                 TYPE RESPONSE  FOR REPORTED EARLY ZJS_R_AgencyTP.
...
CLASS ltc_agency_saver DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    ...
    DATA:
      mapped   TYPE RESPONSE FOR MAPPED   LATE ZJS_R_AgencyTP,
      reported TYPE RESPONSE FOR REPORTED LATE ZJS_R_AgencyTP.
...
CLASS ltc_agency_saver IMPLEMENTATION.
  ...
  METHOD latenumbering_idempotency.
    CONSTANTS: cv_agencyid TYPE ZJS_R_AgencyTP-AgencyId VALUE '123'.

    DATA:
      act_mapped_line LIKE LINE OF mapped-Agency,
      exp_mapped_line LIKE LINE OF mapped-Agency.

    mapped-Agency = VALUE #( ( AgencyId = cv_agencyid ) ).
  ...
  METHOD latenumbering_new_number.
    ...
    mapped-Agency = VALUE #( ( %pid = cv_pid ) ).
    ...
    cl_abap_unit_assert=>assert_equals(
        exp = cv_pid
        act = mapped-Agency[ 1 ]-%pid
      ).

    cl_abap_unit_assert=>assert_not_initial( mapped-Agency[ 1 ]-AgencyId ).
  ENDMETHOD.
